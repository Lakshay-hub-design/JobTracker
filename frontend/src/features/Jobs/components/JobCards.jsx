...
const JobCards = ({ job }) => {
  ...
  return (
    <div className="bg-white p-2 rounded-2xl shadow-sm hover:shadow-md transition">
      <div className={`rounded-2xl p-5 ${bgColor}`}>
        ...
        <div className="mt-2">
          ...
          <p className="text-sm text-orange-900 font-medium capitalize">
            {job.company}
          </p>
        </div>
        ...
        <div className="flex justify-between gap-2 mt-8">
          ...
          <div className="flex items-center text-orange-900">
            <MapPin size={16} />
            <p className="text-sm text-gray-500 ml-2">{job.location}</p>
          </div>
        </div>
      </div>
      <button onClick={() => navigate(`/job/${job._id}`)} className="flex items-center justify-center px-4 py-2 bg-orange-600 w-full mt-2 rounded-full text-white font-semibold cursor-pointer active:scale-95 transition">
        Details
