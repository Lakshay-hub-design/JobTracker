...
const RecentJobs = ({jobs}) => {
  ...
  return (
<div className="bg-white dark:bg-[#151312] rounded-3xl p-4 md:p-5 shadow-md">
      ...
      <div key={index}>
        ...
        <div className="md:hidden bg-gray-50 dark:bg-[#1D1B1A] p-4 rounded-xl space-y-3">
          ...
          <div className="flex justify-between items-center text-sm">
            ...
            <span className={`px-3 py-1 rounded-full font-medium capitalize ${getStatusStyle(job.status)}`}>
              {job.status}
            </span>
        ...
        <div className="hidden md:grid grid-cols-4 items-center px-4 py-3 hover:bg-gray-50 transition rounded-lg">
          ...
          {/* Status */}
          <div>
            <span className={`px-3 py-1 text-sm rounded-full font-medium capitalize ${getStatusStyle(job.status)}`}>
              {job.status}
            </span>
          </div>
